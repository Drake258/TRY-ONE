...
export default async function AdminServicesPage() {
  ...
  const categoryColors: Record<string, string> = {
    ...
    installation: "bg-blue-500/10 text-blue-400",
    ...
    <div className="p-8">
      ...
      {allServices.length === 0 ? (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-16 text-center">
          ...
          <Link href="/admin/services/new" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl transition">
            Add First Service
          </Link>
        </div>
      ) : (
              ...
              <tbody>
                {allServices.map((service) => (
                  <tr key={service.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-blue-400 font-semibold">₵{service.price.toFixed(2)}</span>
                    </td>
                    <td className="px-6 py-4">
